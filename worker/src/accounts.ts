...
export async function handleAccount(
  ...
  const status = url.searchParams.get("status") === "in_review" ? "in_review" : "all";
  const artifacts = await env.ACCOUNTS.prepare(
    `SELECT id,title,summary,artifact_type,status,current_version,current_size,history_size,updated_at
       FROM artifacts WHERE owner_id=? AND (?='all' OR status=?) AND (?='' OR title LIKE ? OR summary LIKE ?)
       ORDER BY updated_at DESC LIMIT 100`,
  ).bind(user.id, status, status, query, `%${query}%`, `%${query}%`).all();
  ...
  return new Response(
    page(
      `<main><div class="hero-row"><div><div class="eyebrow">Private library</div><h1>Work worth keeping.</h1><p class="identity"><strong>${esc(user.email)}</strong> · Agents create and collaborate. You stay in control.</p></div><div class="stats"><div class="stat"><b>${usage.documents}/${FREE_DOCUMENT_LIMIT}</b><span>Documents</span></div><div class="stat"><b>${artifactCount}</b><span>Artifacts</span></div><div class="stat"><b>${formatBytes(usage.totalBytes)} / 1 GB</b><span>Stored incl. history</span></div></div></div><div class="mode-tabs"><a class="active" href="/">Artifacts</a><a href="#encrypted">Encrypted links</a></div><form class="library-tools" method="get" action="/"><input name="q" value="${esc(query)}" placeholder="Search Artifact titles and summaries"><select name="status"><option value="all"${status === "all" ? " selected" : ""}>All Artifacts</option><option value="in_review"${status === "in_review" ? " selected" : ""}>Needs review</option></select><button>Search</button></form><section class="artifact-grid">${artifactCards || '<div class="empty"><div class="empty-mark">◇</div><h2>No matching Artifacts</h2><p>Connect an Agent token and create a permanent Artifact through the API.</p><div class="actions" style="justify-content:center"><a class="button primary" href="https://vnsh.dev/llms.txt">Connect an Agent</a></div></div>'}</section><div id="encrypted" class="section-head"><h2>Encrypted links</h2><span>Host-blind · keep the original URL key</span></div><div class="notice"><span class="shield">◇</span><div><b>Two explicit modes.</b> Account Artifacts are discoverable and readable by vnsh. These encrypted workspaces keep their key only in the URL.</div></div><section class="grid">${cards || '<div class="empty"><h2>No encrypted workspaces saved</h2><p>Quick encrypted sharing remains account-free.</p></div>'}</section><div class="section-head"><h2>Devices and tokens</h2><span>${sessions.results.length} active</span></div><section class="grid">${sessionCards}</section><div class="actions"><form method="post" action="/api/account/token"><input name="label" maxlength="60" placeholder="Token name (for example, Cursor laptop)" required><button class="button primary">Create CLI / agent token</button></form><form method="post" action="/sessions/revoke-others"><button>Revoke all other sessions</button></form><a class="button" href="https://vnsh.dev/llms.txt">Agent setup guide</a><form method="post" action="/logout"><button class="link-button">Sign out</button></form></div><div class="section-head"><h2>Delete account</h2><span>Permanent and irreversible</span></div><div class="notice"><span class="shield">!</span><div><b>Deletes every saved document, retained version, device and token.</b><form method="post" action="/account/delete"><label for="delete-email">Type ${esc(user.email)} to confirm</label><input id="delete-email" name="email" type="email" autocomplete="off" required><button class="delete">Delete account and all documents</button></form></div></div></main>`,
    ),
    { headers: htmlHeaders },
  );
}
